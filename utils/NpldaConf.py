...
class E2EConf:
    def __init__(self, configfile):
        ...
        if config.has_option('Paths', 'train_spk2utt_list'):
            self.train_spk2utt_list = config['Paths']['train_spk2utt_list'].split(',')
        if config.has_option('Paths', 'train_utt2dur'):
            self.train_utt2dur = config['Paths']['train_utt2dur']
        if config.has_option('Paths', 'training_data_trials_list'):
            self.training_data_trials_list = config['Paths']['training_data_trials_list'].split(',')
        self.validation_trials_list = config['Paths']['validation_trials_list'].split(',')
        self.test_trials_list = config['Paths']['test_trials_list'].split(',')
        ...
        if config.has_option('Training', 'batch_size'):
            self.batch_size = int(config['Training']['batch_size'])
        if config.has_option('Training', 'total_batch_duration'):
            self.total_batch_duration = int(config['Training']['total_batch_duration'])
